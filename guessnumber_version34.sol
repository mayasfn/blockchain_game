...
contract GuessNumber {
    ...
    function revealSecret(uint256 roomNumber, uint256 secret) external {
        Room storage room = rooms[roomNumber];
        require(room.exists, "Room does not exist");
        bool allRoundsPlayed = room.guesses.length == room.maxRounds;
        bool earlyWin = room.feedbacks.length > 0 && room.feedbacks[room.feedbacks.length - 1] == 0;
        require(allRoundsPlayed || earlyWin, "Game not finished");
